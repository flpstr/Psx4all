...
INLINE void GPUDMA_INT(u32 uCycles)
{
	...
	psxRegs->intCycle[3+24+1] = uCycles;
}
...
void psxDma4(u32 madr, u32 bcr, u32 chcr) { // SPU
	...
	psx4all_prof_start(PROFILER_DMA4);
	switch (chcr) {
	...
	DMA_INTERRUPT(4);
	psx4all_prof_end(PROFILER_DMA4);
}
...
void psxDma2(u32 madr, u32 bcr, u32 chcr) { // GPU
	...
	psx4all_prof_start(PROFILER_DMA2);
	switch(chcr) {
...
//			GPUDMA_INT((size / 4) / 2);
//			psx4all_prof_end(PROFILER_DMA2);
//			return;
			break;
...
#ifdef PSXDMA_LOG
			PSXDMA_LOG("*** DMA 2 DONE! - GPU dma chain *** %lx addr = %lx size = %lx\n", chcr, madr, bcr);
#endif
			break;
...
void psxDma6(u32 madr, u32 bcr, u32 chcr) {
	u32 *mem = (u32 *)PSXM32(madr);

	psx4all_prof_start(PROFILER_DMA6);
#ifdef PSXDMA_LOG
	...
	if (chcr == 0x11000002) {
		if (mem == NULL) {
			...
			DMA_INTERRUPT(6);
			psx4all_prof_end(PROFILER_DMA6);
			return;
		}
