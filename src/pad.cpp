...
#endif
	if( (keys & GP2X_SELECT && keys & GP2X_START) || (keys & GP2X_VOL_DOWN) )
	{
#ifdef GP2X_SDL
		// Menu system
		u32 w = sdlscreen->w;
...
*/
	...
	{
		// Overclock
    u32 newClock = (PsxClockMult<400) ? (PsxClockMult+10) : PsxClockMult;
	...
	{
	// Underclock
    u32 newClock = (PsxClockMult>2) ? (PsxClockMult-10) : PsxClockMult;
    UpdateClock (newClock);
    gp2x_printf(NULL, 0, 8, "PSX CLOCK %d %%            ", newClock);
		gp2x_video_flip();
		gp2x_timer_delay(250);
