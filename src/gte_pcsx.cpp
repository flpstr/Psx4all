...
#define gteMAC0    psxRegs->CP2D.r[24].d
...
__inline void MTC2(unsigned long value, int reg) {
	...
	switch(reg) {
		case 8: case 9: case 10: case 11:
			psxRegs->CP2D.r[reg].d = (short)value;
			break;
		...
		case 16: case 17: case 18: case 19:
			psxRegs->CP2D.r[reg].d = (value & 0xffff);
			break;

		case 28:
			psxRegs->CP2D.r[28].d = value;
			gteIR1 = ((value      ) & 0x1f) << 7;
...
//			gteIR3 = ((value >> 10) & 0x1f) << 4;
		...
		case 30:
			psxRegs->CP2D.r[30].d = value;

			a = psxRegs->CP2D.r[30].d;
			if (a > 0) {
				int i;
				for (i=31; (a & (1 << i)) == 0 && i >= 0; i--);
				psxRegs->CP2D.r[31].d = 31 - i;
			} else if (a < 0) {
				int i;
				a^= 0xffffffff;
				for (i=31; (a & (1 << i)) == 0 && i >= 0; i--);
				...
				psxRegs->CP2D.r[31].d = 32;
			}
			break;
		
		default:
			psxRegs->CP2D.r[reg].d = value;
	}
}
...
void gteCFC2(u32 code) {
	...
	if (!_Rt_) return;
	psxRegs->GPR.r[_Rt_] = psxRegs->CP2C.r[_Rd_].d;
}
...
void gteCTC2(u32 code) {
	...
	psxRegs->CP2C.r[_Rd_].d = psxRegs->GPR.r[_Rt_];
}
